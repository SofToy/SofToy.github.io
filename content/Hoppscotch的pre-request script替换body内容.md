...
看了https://github.com/hoppscotch/hoppscotch/issues/3904，误解成没办法改，打算下源码自己改进再构建下。

### 发现没必要
发现我这个需求完全没必要去改源码，有另一种workaround，就是用变量，因为body里打入<<没见下拉提示，以为没有这个功能，实际上是有的。
...
虽然左下会有提示报错找不到变量，但是执行前会在`getFinalEnvsFromPreRequest`赋值变量。